...
export default function getProducts({ data }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const productData_Pagination =
        (await getProducts_PaginationData(query)) || [];
            ...
            <div className="py-2 inline-block min-w-full ">
              <div className="overflow-hidden">
                <table className="table-auto min-w-full text-center">
                  <thead className="border-b-4 border-solid border-neutral-400">
                    <tr>
                  ...
                  <tbody>
                    {finalData?.map((product) => {
                      return (
                        <tr
                          className="border-b-4 border-solid border-neutral-400"
                          key={product.id}
                        >
                          ...
                          <td className="text-sm  font-light  py-4 whitespace-nowrap">
                            <Link
                              href={`updateProducts/${product.id}`}
                              className="bg-orange-500 mx-2 hover:bg-orange-900   font-bold py-2 px-4 rounded-full"
                            >
                              Update
                            </Link>
                            <button
                              onClick={() => handleDelete(product.id)}
                              className="bg-red-500 mx-2 hover:bg-red-900   font-bold py-2 px-4 rounded-full"
                            >
                              Delete
                            </button>
                          </td>
